...
let ext = {
    ...
    smoothScrollPage: smoothScrollPage,
 ...
 * <br> {string} - app package name or app name -- eg: 'com.eg.android.AlipayGphone' or 'Alipay'
...
/**
 * Refresh screen widgets or current package by a certain strategy
 * @global
 * @param {string} [strategy]
 * <br>
 *     -- /widgets?|alert/ - alert() + text(%ok%).click() - may refresh widgets only
 *     -- 'page'|'recent[s]'|*DEFAULT*|*OTHER* - recents() + back() - may refresh currentPackage() <br>
 * @param {object} [params]
 * @param {string} [params.custom_alert_text='Alert for refreshing widgets']
 * @param {string} [params.current_package=currentPackage()]
 * @param {boolean} [params.debug_info_flag]
 */
function refreshWidgets(strategy, params) {
    let _params = params || {};
    let _stg = strategy || '';
    ...
    if (_stg.match(/widgets?|alert/)) {
        descMatches(/.*/).exists(); // useful or useless ?

        let alert_text = _params.custom_alert_text || 'Alert for refreshing widgets';
 ...
 */
function smoothScrollPage(direction, callback, options) {
    let _cbk = callback || {};
    ...
    if (_is_left) {
        ...
        throw Error('Cannot parse direction for smoothScrollPage()');
    }
