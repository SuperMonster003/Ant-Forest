...
定时任务先放一放了  
:persevere:  
正在奋力让解锁配置向导同时适配 `4.1.1 alpha2` 和 `4.1.0 alpha5`  
:weary:  
然后再写全局配置向导  
...
"Auto.js"版本 `4.1.0 Alpha5` (其他版本暂未测试)  
...
* 适应恶劣条件  
** 脚本在恶劣条件下仍可正常运行或识别异常 (网络条件较差/~~意外来电~~/~~支付宝异常退出~~/~~广告弹窗~~等)  
* ~~配置向导~~  
...
* `修复` ~~解锁配置向导在 `4.1.1 alpha2` 的兼容问题~~ (今天是周六 再等等我)
