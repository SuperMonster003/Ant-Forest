...
* `修复` 图形配置页面保存后可能导致解锁模块配置丢失的问题  
* `修复` Floaty悬浮窗结果展示倒计时可能卡住的问题  
* `修复` 图形配置页面存在依赖关系的子页面可能失去页面关联的问题  
* `修复` 图形配置页面连续快速点击可能导致页面切换异常的问题  
...
* `优化` 完善图形配置页面版本历史更新内容正则匹配的"pattern"参数  
* `优化` 部分工具函数模块化  
·  setDeviceProto()  
·  vibrateDevice()  
·  sleepSafe()  
...
* `优化` 图形配置页面Activity结束时的内存回收方法  
* `优化` 图形配置页面初始化期间切换页面的响应速度  
* `优化` 图形配置页面强制竖屏防止布局错乱及页面切换异常  
