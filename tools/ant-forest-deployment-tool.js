...
let $_cvt = function () {
    /**
     * @typedef {number|string} $_cvt$src
     * @typedef {string} $_cvt$init_unit
     * @typedef {{
     ...
     * @param {$_cvt$src} src
     * @param {$_cvt$init_unit|'B'|'KB'|'MB'|'GB'|'TB'|'PB'|'EB'|'ZB'|'YB'} [init_unit='B']
     * @param {$_cvt$options} [options]
     * @example
...
let $appx = {
    ...
    backupProject(callback, options) {
        ...
        let _bak_file_name = $_cvt.date(_now_ts, 'yyMMddhhmmss') + '-' + _local_ver_hex + '.zip';
...
let $dialogsx = {
    ...
    build(props) {
        ...
        function _parseBtnText(text) {
            return _$dialogsx._text._btn[text] || text;
        }
    },
    /**
    ...
    buildFlow(config) {
        ...
        return Object.create(this.builds([
            ...
            /** @typedef {JsDialog$ & BuildFlowExtended} BuildFlowExtendedJsDialog */
            {
                    ...
                    value() {
                        ...
                        config.steps.forEach((step, idx) => {
                            _promise = _promise.then((value) => {
                                if (global._$_dialog_flow_interrupted) {
                                    throw Error(_$dialogsx._text.user_interrupted);
                                }
                                let _fin = (result) => {
                        ...
                        _promise = _promise.then((res) => {
                            ...
                            _$dialogsx.setProgressColorTheme(this, 'finish');

                            _setStepsFinished.call(this, 'all');

                            this.removeAllListeners('positive');
                            this.setActionButton('positive', _$dialogsx._text._btn.F);
                            this.on('positive', d => d.dismiss());

                            let _title = config.success_title;
                            _title && _$dialogsx.setTitleText(this, _title);

                            let _cont = config.success_content;
                            _cont && _$dialogsx.appendContentText(this, '\n\n' + _cont);
                        ...
                        _promise.catch((err) => {
                            _$dialogsx.setProgressColorTheme(this, 'error');

                            this.removeAllListeners('positive');
                            this.setActionButton('positive', _$dialogsx
                                ._text._btn[config.on_interrupt_btn_text || 'B']
                            );
                            this.on('positive', d => d.dismiss());

                            _$dialogsx.alertContent(this, err, 'append');
                ...
                setFailureData: {
                    value(error) {
                        this.setActionButton('positive', _$dialogsx._text._btn.B);
                        this.removeAllListeners('positive');
                        this.on('positive', d => d.dismiss());
        ...
        function _setStepsFinished(ctr) {
            ...
            let _cont = _$dialogsx.getContentText(this);
            let _rex = /^(. )(\d)(?=\.)/gm;
            _$dialogsx.setContentText(this, _cont.replace(_rex, ($0, $1, $2) => (
                ($2 <= _ctr ? '\u2714 ' : $1) + $2
            )));
        }
...
$dialogsx.builds(['项目部署',
    ...
    $dialogsx.linkify($dialogsx.builds(['关于项目',
        '- 功能简介 -' + '\n' + [
 ...
 */
function _waitForAction(condition, timeout_or_times, interval) {
